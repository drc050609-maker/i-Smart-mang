...
export function AddLeadChildDialog({ leadId }: { leadId: number }) {
  const { t } = useLanguage();
  const idPrefix = useId();
  const [open, setOpen] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const formRef = useRef<HTMLFormElement>(null);
  const [state, formAction, pending] = useActionState(createLeadChild, initialState);
                ...
                <div>
                  ...
                  <div className="mt-2">
                    <textarea
                      ...
                      placeholder={t("leads.backgroundPlaceholder")}
                    />
                  </div>
                </div>
