...
import {
  makeLeadOfficialStudent,
  makeTrialStudentOfficial,
...
export function MakeOfficialStudentDialog(props: InquiryProps | TrialProps) {
  ...
  const [state, formAction, pending] = useActionState(
    props.kind === "inquiry" ? makeLeadOfficialStudent : makeTrialStudentOfficial,
    initialState,
  );
            ...
            >
              <div className="absolute top-0 right-0 hidden pt-4 pr-4 sm:block">
                <button
                  ...
                  onClick={closeDialog}
                  disabled={pending}
                  className="rounded-md text-gray-400 hover:text-gray-500 focus:outline-2 focus:outline-offset-2 focus:outline-indigo-600 disabled:opacity-50 dark:hover:text-gray-300"
                >
              ...
              <p className="mt-2 text-sm text-gray-500 dark:text-gray-400">
                {props.kind === "inquiry"
                  ? t("leads.makeOfficialDescription", { name: props.name })
                  : t("leads.makeTrialOfficialDescription", { name: props.name })}
              </p>
              ...
              <form action={formAction} className="mt-6 space-y-4">
                ...
                <div>
                  <label
                    htmlFor={`official-credits-${props.kind}-${props.kind === "inquiry" ? props.leadId : props.studentId}`}
                  ...
                  <div className="mt-2">
                    <input
                      id={`official-credits-${props.kind}-${props.kind === "inquiry" ? props.leadId : props.studentId}`}
                  ...
                  <p className="mt-1 text-xs text-gray-500 dark:text-gray-400">
                    {props.kind === "inquiry"
                      ? t("leads.makeOfficialHelp")
                      : t("leads.makeTrialOfficialHelp")}
                  </p>
                </div>
                ...
                <div className="flex justify-end gap-3">
                  <button
                    ...
                    onClick={closeDialog}
                    disabled={pending}
                    className="rounded-md bg-white px-3 py-2 text-sm font-semibold text-gray-900 shadow-xs inset-ring inset-ring-gray-300 hover:bg-gray-50 disabled:opacity-50 dark:bg-white/10 dark:text-white dark:shadow-none dark:inset-ring-white/10 dark:hover:bg-white/20"
                  >
                    {t("common.cancel")}
                  </button>
                  <button
                  ...
                  >
                    ...
                    {pending ? t("common.processing") : t("leads.makeOfficial")}
                  </button>
                </div>
              </form>
            </DialogPanel>
