...
export function TeacherMultiCombobox({
  id,
  teachers,
  selected,
  onChange,
  ...
  const selectedIds = useMemo(
    () => new Set(selected.map((teacher) => teacher.id)),
    [selected],
  );
  const sortedTeachers = useMemo(() => sortTeachers(teachers), [teachers]);
  const filteredTeachers = useMemo(
    () => filterTeachersByQuery(sortedTeachers, query),
  ...
  const handleTeacherCreated = useCallback(
    ...
    [onChange, onTeacherAdded, selected, selectedIds],
  );

  function toggleTeacher(teacher: TeacherOption) {
    if (selectedIds.has(teacher.id)) {
      onChange(selected.filter((item) => item.id !== teacher.id));
      return;
    }
  ...
  function removeTeacher(teacherId: number) {
    onChange(selected.filter((item) => item.id !== teacherId));
  }
  ...
  return (
    <div className="space-y-3">
            ...
            >
              ...
              >
                <span className="sr-only">
                  {t("common.remove")} {formatTeacherName(teacher)}
                </span>
      ...
      <Combobox
        as="div"
        value={null}
        onChange={(teacher: TeacherOption | null) => {
          if (teacher) {
      ...
      >
        <div className="relative">
          <ComboboxInput
            ...
            placeholder={t("common.searchAndSelectTeachers")}
            autoComplete="off"
          />
