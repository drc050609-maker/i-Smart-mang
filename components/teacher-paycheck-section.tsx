...
import {
  openTeacherPaycheckPdf,
  type TeacherPaycheckPdfLine,
} from "@/lib/teacher-paycheck-pdf";
...
const pdfButtonClassName =
  "rounded-md bg-white px-3 py-2 text-sm font-semibold text-gray-900 shadow-xs inset-ring inset-ring-gray-300 hover:bg-gray-50 dark:bg-white/10 dark:text-white dark:shadow-none dark:inset-ring-white/10 dark:hover:bg-white/15";
...
function PaycheckPeriodCard({
  ...
  function handleDownloadPdf() {
    ...
    const monthLabel = formatStatementMonth(period.year, period.month, language);
    const pdfLines: TeacherPaycheckPdfLine[] = period.classLines.map((line) => {
      const savedLine = savedPaycheck?.lines.find(
        (saved) => saved.groupKey === line.groupKey,
      );
      const sessionCount = isRecorded
        ? (savedLine?.sessionCount ?? line.sessionCount)
        : line.sessionCount;
      const rateCents = isRecorded
      ...
      const subtotalCents = isRecorded
        ...
        : sessionCount * rateCents;

      return {
        classLabel: formatSubjectWithGradeTier(
          line.subject,
          line.gradeTier,
          language,
        ),
        sessions: String(sessionCount),
        rate: formatStatementAmountCents(rateCents),
        subtotal: formatStatementAmountCents(subtotalCents),
      };
    });

    const totalCents = isRecorded
      ? savedPaycheck.effectiveAmountCents
      : liveTotalCents;

    const opened = openTeacherPaycheckPdf(
      {
        title: `${teacherName} — ${t("common.teacherPaycheck")}`,
        subtitle: isRecorded
          ? `${monthLabel} · ${t("common.recordedAt", {
              date: formatRecordedAt(savedPaycheck.createdAt, language),
            })}`
          : monthLabel,
        classLabel: t("common.class"),
        sessions: t("common.classesColumn"),
        rate: t("common.ratePerClass"),
        subtotal: t("common.subtotal"),
        total: t("common.totalPaycheck"),
        totalAmount: formatStatementAmountCents(totalCents),
        ...
        <div className="mt-4">
          ...
          <div className="flex flex-wrap items-center gap-3">
            ...
            <button
              ...
              onClick={handleDownloadPdf}
              disabled={liveTotalCents <= 0}
              className={`${pdfButtonClassName} disabled:cursor-not-allowed disabled:opacity-50`}
            >
              {t("common.downloadPdf")}
            </button>
          </div>
