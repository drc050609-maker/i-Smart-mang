-- Intake fields for trial bookings: gender, parent name, weekday/weekend preference.
-- Also persist phone + address, campus trial pricing, and editable duration.

ALTER TABLE public.students
  ADD COLUMN IF NOT EXISTS gender text,
  ADD COLUMN IF NOT EXISTS parent_name text,
  ADD COLUMN IF NOT EXISTS trial_time_preference text;
...
ALTER TABLE public.students
  DROP CONSTRAINT IF EXISTS students_trial_time_preference_check;
ALTER TABLE public.students
  ADD CONSTRAINT students_trial_time_preference_check
  CHECK (
    trial_time_preference IS NULL
    OR trial_time_preference IN ('weekday', 'weekend')
  );

COMMENT ON COLUMN public.students.gender IS
  'Student gender collected on trial signup: male or female.';
COMMENT ON COLUMN public.students.parent_name IS
  'Parent or guardian name collected on trial signup.';
COMMENT ON COLUMN public.students.trial_time_preference IS
  'Preferred trial window: weekday or weekend.';

DROP FUNCTION IF EXISTS public.book_trial_class(
  text, text, date, text, text, bigint, date, time without time zone, text, text
);
