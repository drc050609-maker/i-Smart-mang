-- Staff-chosen trial fee: $25 (paid) or $0 (current promo).
-- $0 must persist on class_payments and statement_entries.
-- Paid trials keep campus trial_teacher_pay_cents; $0 promo sets teacher pay to 0.

ALTER TABLE public.class_payments
  DROP CONSTRAINT IF EXISTS class_payments_amount_cents_check;
ALTER TABLE public.class_payments
  ADD CONSTRAINT class_payments_amount_cents_check
  CHECK (amount_cents >= 0);

ALTER TABLE public.statement_entries
  DROP CONSTRAINT IF EXISTS statement_entries_amount_cents_check;
ALTER TABLE public.statement_entries
  ADD CONSTRAINT statement_entries_amount_cents_check
  CHECK (amount_cents >= 0);
...
BEGIN
  ...
  SELECT
    ...
    c.subject,
    c.lesson_type
  INTO v_student_name, v_class_subject, v_lesson_type
  FROM students AS s
  ...
  IF p_amount_cents > 0 THEN
    NULL;
  ELSIF p_amount_cents = 0 AND v_lesson_type = 'trial' THEN
    NULL;
  ELSE
...
CREATE OR REPLACE FUNCTION public.book_trial_class(
  ...
  p_trial_price_cents integer DEFAULT 2500
)
RETURNS TABLE (
  student_id bigint,
  class_id bigint,
...
DECLARE
  ...
  v_price_cents integer := coalesce(p_trial_price_cents, 2500);
...
BEGIN
  ...
  IF v_price_cents NOT IN (0, 2500) THEN
    RAISE EXCEPTION 'Trial fee must be $25 or $0';
  END IF;
  ...
  SELECT coalesce(l.trial_teacher_pay_cents, 1500)
  INTO v_teacher_pay_cents
  FROM locations AS l
  WHERE l.id = v_location_id;

  IF v_price_cents = 0 THEN
    v_teacher_pay_cents := 0;
  END IF;
  ...
  IF v_price_cents = 0 THEN
    IF v_notes IS NULL OR char_length(trim(v_notes)) = 0 THEN
      v_notes := 'Trial class signup (promo $0)';
    ELSE
      v_notes := v_notes || ' · Promo $0';
    END IF;
  END IF;
